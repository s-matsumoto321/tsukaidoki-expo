...
function StackedAreaChart({
  svgW, chartH = 170, yearCount, stacks, colors,
}: {
...
export default function PoolScreen() {
  ...
  const poolItems = useMemo<PoolItem[]>(() => {
    ...
    return POOL_ITEMS.map(item => {
      ...
      return {
        ...
        rate: savedRates[id] ?? 0,
        monthly: currentEntry?.monthlyAmounts[id] ?? 0,
      };
    });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      {/* ヘッダー */}
      <View style={[ps.header, { paddingTop: insets.top + 10 }]}>
        <Pressable onPress={() => router.back()} hitSlop={12}>
          <Text style={ps.backTxt}>‹ ホーム</Text>
        </Pressable>
        <View style={ps.headerRow}>
          <Text style={ps.headerTitle}>プール金</Text>
          <View style={{ alignItems: 'flex-end' }}>
